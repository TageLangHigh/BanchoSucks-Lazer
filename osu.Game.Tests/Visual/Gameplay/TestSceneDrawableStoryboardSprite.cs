    ...
    public partial class TestSceneDrawableStoryboardSprite : SkinnableTestScene
    {
        ...
        public void TestPassingFailingTriggers()
        {
            ...
            AddStep("create sprites", () => SetContents(_ =>
            {
                ...
                var passTrigger = sprite.AddTriggerGroup(@"Passing", double.NegativeInfinity, double.PositiveInfinity, 0);
                passTrigger.AddFlipV(Easing.None, 0, 0, false, false);
                var failTrigger = sprite.AddTriggerGroup(@"Failing", double.NegativeInfinity, double.PositiveInfinity, 0);
        ...
        public void TestHitObjectHitTrigger()
        {
            ...
            AddStep("create sprites", () => SetContents(_ =>
            {
                ...
                var loop = sprite.AddLoopingGroup(Time.Current, 100);
                loop.AddAlpha(Easing.None, 0, 10000, 1, 1);
                var hitObjectHitTrigger = sprite.AddTriggerGroup(@"HitObjectHit", double.NegativeInfinity, double.PositiveInfinity, 0);
        ...
        [Test]
        public void TestHitObjectSampleTrigger()
        {
            AddStep("allow all lookups", () =>
            {
                storyboard.UseSkinSprites = false;
            ...
            AddStep("create sprites", () => SetContents(_ =>
            {
                ...
                var loop = sprite.AddLoopingGroup(Time.Current, 100);
                loop.AddAlpha(Easing.None, 0, 10000, 1, 1);
                var whistleTrigger = sprite.AddTriggerGroup(@"HitSoundWhistle", double.NegativeInfinity, double.PositiveInfinity, 0);
                whistleTrigger.AddColour(Easing.None, 0, 2000, Colour4.Red, Colour4.White);
                var drumTrigger = sprite.AddTriggerGroup(@"HitSoundDrum", double.NegativeInfinity, double.PositiveInfinity, 0);
                drumTrigger.AddScale(Easing.None, 0, 2000, 1.2f, 1f);
            ...
            AddStep("play normal-hitnormal", () => ((Bindable<ISampleInfo[]>)gameplayState.LastPlayedSamples).Value =
            [
                new HitSampleInfo(HitSampleInfo.HIT_NORMAL)
            ]);
            AddAssert("nothing happens", () => this.ChildrenOfType<DrawableStoryboardSample>().All(s => s.Scale == Vector2.One));

            AddStep("play drum-hitnormal", () => ((Bindable<ISampleInfo[]>)gameplayState.LastPlayedSamples).Value =
            [
                new HitSampleInfo(HitSampleInfo.HIT_NORMAL, HitSampleInfo.BANK_DRUM)
            ]);
            AddAssert("scale changed", () => this.ChildrenOfType<DrawableStoryboardSample>().All(s => s.Scale.X > 1));
            AddUntilStep("scale eventually reverts", () => this.ChildrenOfType<DrawableStoryboardSample>().All(s => s.Scale == Vector2.One));

            AddStep("play soft-hitnormal + soft-hitwhistle", () => ((Bindable<ISampleInfo[]>)gameplayState.LastPlayedSamples).Value =
            [
            ...
            AddStep("play drum-hitnormal + drum-hitwhistle", () => ((Bindable<ISampleInfo[]>)gameplayState.LastPlayedSamples).Value =
            [
                new HitSampleInfo(HitSampleInfo.HIT_NORMAL, HitSampleInfo.BANK_DRUM),
                new HitSampleInfo(HitSampleInfo.HIT_WHISTLE, HitSampleInfo.BANK_DRUM),
            ]);
            AddAssert("colour and scale changed",
                () => this.ChildrenOfType<DrawableStoryboardSprite>().All(s => s.Scale.X > 1 && s.Colour != Colour4.White));
            AddUntilStep("colour and scale eventually revert",
                () => this.ChildrenOfType<DrawableStoryboardSprite>().All(s => s.Scale == Vector2.One && s.Colour == Colour4.White));
        }
